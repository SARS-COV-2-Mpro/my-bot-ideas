    ...
    steps:
      ...
      - name: Run pusher (always-push + clear logs)
        ...
        run: |
          ...
          const depth1pUSD = (depth, mid) => {
            if (!depth?.asks?.length || !depth?.bids?.length || !(mid > 0)) return 0;
            const minBid = mid * 0.99, maxAsk = mid * 1.01;
            let usdBid = 0, usdAsk = 0;
            for (const [ps, qs] of depth.bids) { const p=+ps, q=+qs; if (p < minBid) break; usdBid += p*q; }
            for (const [ps, qs] of depth.asks) { const p=+ps, q=+qs; if (p > maxAsk) break; usdAsk += p*q; }
            return Math.min(usdBid, usdAsk);
          };
          ...
          const UA="gh-actions-ideas-mexc-ultimate/7.4-stable-tf (+https://github.com/)";
